...
import { ButtonGroup, Button, Icon, Avatar, SocialIcon, Rating, Input } from 'react-native-elements';
import { Image,TextInput, Text, View, FlatList, ActivityIndicator, TouchableOpacity, ScrollView, Dimensions, SafeAreaView, ImageBackground } from 'react-native';
      ...
      <TouchableOpacity onPress={() => navigation.navigate('EventDetaille', { id: item.id, title: 'Math Kendy' })}>
        <View style={{ flex: 1, flexDirection: 'row', marginVertical: 10, minHeight: 250, marginHorizontal: 10, borderColor: '#dbdbdb', borderWidth: 1 }} >
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#ededed', paddingHorizontal: 8 }}>
            ...
            <Avatar size="large" source={{ uri: 'https://randomuser.me/api/portraits/men/' + item.id + '.jpg' }} rounded />
...
const HorizontalMapFiter = ({ item, categories }) => {
  return <View style={{
    ...
    <FlatList
      ...
      renderItem={({ item }) =>
        <TouchableOpacity onPress={() => this.show(item.name)}>
          <View style={{
            borderColor: '#ccc', borderWidth: 0, borderWidth: 1,
            ...
            <Text style={{
              textTransform: 'capitalize', textAlignVertical: 'center', fontWeight: 'bold', textAlign: 'center', color: '#000',
              fontWeight: 'normal'
            }}> {item.name} </Text>
          </View>
        </TouchableOpacity>
